	...
	DatePicker = function () {
		var ids = {},
			...
			fill = function(el) {
				...
				for(var i = 0; i < options.calendars; i++) {
					...
					while(cnt < 42) {
						...
						var val = date.valueOf() + 12*3600000; // set date at noon to make sure that the comparisons below does not fail because of a slightly wrong timestamp (e.g.: milliseconds are not cleared)
			...
			getEndOfWeek = function(date, setTimeAlso) {
				...
				if (setTimeAlso) {
					return sunday.setHours(23,59,59,0);
				}
				else {
					return sunday;
				}
							...
							// clicked a day cell
								...
								switch (options.mode) {
									...
									case 'tworanges':
										...
										var second = first + 1;
										var now, modulo,monday,sunday,d1,d2,tmpSwap;
										if (options.multiweekMode)
										{
											monday = getStartOfWeek(tmp, true); // returns monday
											if (current == first) {
												// first click: set to the start of the week
												options.date[first] = monday.valueOf();
											}
											// get the very end of the week clicked
											sunday = getEndOfWeek(monday, true); // returns sunday 
											val = sunday.valueOf();
											options.date[second] = val;
											if (current == second) {
												// swap weeks if necessary 
												d1 = new Date(options.date[first]);
												d2 = new Date(options.date[second]);
												if (d2 < d1) {
													// we must recalculate monday and sunday since we need monday of first week and sunday of last week and not viceversa
													monday = getStartOfWeek(d2, true);
													sunday = getEndOfWeek(d1, true);
													options.date[second] = sunday.valueOf();
													options.date[first] = monday.valueOf();
												}
											}
											...
											modulo = options.mode == 'range' ? 2 : 4;
											options.lastSel = (current + 1) % modulo;
										}
										else if (options.weeklyMode)
										{
											monday = getStartOfWeek(tmp, true);
											sunday = getEndOfWeek(tmp, true);
										...
										else
										{
											...
											if (current == second) {
												...
												d2 = new Date(options.date[second]);
												if (d2 < d1) {
													tmpSwap = (d2.setHours(0,0,0,0)).valueOf();
													options.date[second] = (d1.setHours(23,59,59,0)).valueOf();
