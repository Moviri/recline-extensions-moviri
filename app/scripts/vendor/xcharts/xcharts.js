...
_.defaults(xChart.prototype, {
  ...
  _drawAxes: function () {
    ...
    if (labels.length > (self._width / 80)) {
      ...
      labels = uniqueLabels;
      // if, after dupe removal, we still have too many labels, apply usual tickHintX filtering
      if (labels.length > (self._width / 80)) {
	      if (xTicks > 0)
		      d3.selectAll(labels)
		        .filter(function (d, i) {
  ...
  _enter: function (vis, storage, data, className) {
    var self = this,
      callbacks = {
        ...
        mouseoverAnnotation: self._options.mouseoverAnnotation,
        mouseoutAnnotation: self._options.mouseoutAnnotation
      };
