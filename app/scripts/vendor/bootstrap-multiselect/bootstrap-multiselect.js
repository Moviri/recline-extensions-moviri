...
!function ($) {
    ...
    function Multiselect(select, options) {
        ...
        this.container = $(this.options.buttonContainer)
            .append('<button type="button" class="'+ this.options.buttonClassFirst +' select-all-button">' + this.options.buttonText($('option:selected', select)) + '</button><button type="button" class="dropdown-toggle ' + this.options.buttonClass + '" data-toggle="dropdown"><b class="caret"></b></button>')
    ...
    Multiselect.prototype = {
        buildDrowdown: function(select, options){
            ...
            $('option', this.select).each($.proxy(function(index, element) {
                ...
                $('ul', this.container).append('<li><a href="javascript:void(0);" style="padding:0;"><label style="margin:0;padding:0px 0px 0px 5px;width:100%;height:100%;cursor:pointer;"><input style="margin-bottom:5px;" type="checkbox" value="' + $(element).val() + '" /> ' + $(element).text() + '</label</a></li>');
            ...
            $('ul li input[type="checkbox"]', this.container).on('change', $.proxy(function(event) {
                ...
                $('button.select-all-button', this.container).html(this.options.buttonText(options));
