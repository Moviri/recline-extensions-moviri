d3v3 = function() {
  var π = Math.PI, ε = 1e-6, d3v3 = {
    version: "3.0.6"
  }, d3v3_radians = π / 180, d3v3_degrees = 180 / π, d3v3_document = document, d3v3_window = window;
  ...
  var d3v3_format_decimalPoint = ".", d3v3_format_thousandsSeparator = ",", d3v3_format_grouping = [ 3, 3 ];
  ...
  try {
    ...
    var d3v3_style_prototype = d3v3_window.CSSStyleDeclaration.prototype, d3v3_style_setProperty = d3v3_style_prototype.setProperty;
  ...
  var d3v3_array = d3v3_arraySlice;
  function d3v3_arrayCopy(pseudoarray) {
    var i = -1, n = pseudoarray.length, array = [];
    while (++i < n) array.push(pseudoarray[i]);
    return array;
  }
  function d3v3_arraySlice(pseudoarray) {
    return Array.prototype.slice.call(pseudoarray);
  }
  try {
    d3v3_array(d3v3_document.documentElement.childNodes)[0].nodeType;
  } catch (e) {
    d3v3_array = d3v3_arrayCopy;
  }
  var d3v3_arraySubclass = [].__proto__ ? function(array, prototype) {
  ...
  d3v3_class(d3v3_Map, {
    has: function(key) {
      return d3v3_map_prefix + key in this;
    },
    get: function(key) {
      return this[d3v3_map_prefix + key];
    },
    set: function(key, value) {
      return this[d3v3_map_prefix + key] = value;
    },
    remove: function(key) {
      key = d3v3_map_prefix + key;
    ...
    forEach: function(f) {
      for (var key in this) {
        if (key.charCodeAt(0) === d3v3_map_prefixCode) {
          f.call(this, key.substring(1), this[key]);
        }
      }
    }
  });
  var d3v3_map_prefix = "\0", d3v3_map_prefixCode = d3v3_map_prefix.charCodeAt(0);
  ...
  d3v3.mean = function(array, f) {
    ...
    if (arguments.length === 1) {
      while (++i < n) if (d3v3_number(a = array[i])) m += (a - m) / ++j;
    } else {
      while (++i < n) if (d3v3_number(a = f.call(array, array[i], i))) m += (a - m) / ++j;
    }
    return j ? m : undefined;
  };
  d3v3.median = function(array, f) {
    if (arguments.length > 1) array = array.map(f);
    array = array.filter(d3v3_number);
  ...
  var d3v3_bisector = d3v3.bisector(function(d) {
    return d;
  });
  d3v3.bisectLeft = d3v3_bisector.left;
  d3v3.bisect = d3v3.bisectRight = d3v3_bisector.right;
  d3v3.nest = function() {
    var nest = {}, keys = [], sortKeys = [], sortValues, rollup;
    function map(array, depth) {
      if (depth >= keys.length) return rollup ? rollup.call(nest, array) : sortValues ? array.sort(sortValues) : array;
      var i = -1, n = array.length, key = keys[depth++], keyValue, object, valuesByKey = new d3v3_Map(), values, o = {};
      while (++i < n) {
  ...
  d3v3.range = function(start, stop, step) {
    ...
    var range = [], k = d3v3_range_integerScale(Math.abs(step)), i = -1, j;
  ...
  d3v3.xhr = function(url, mimeType, callback) {
    var xhr = {}, dispatch = d3v3.dispatch("progress", "load", "error"), headers = {}, response = d3v3_identity, request = new (d3v3_window.XDomainRequest && /^(http(s)?:)?\/\//.test(url) ? XDomainRequest : XMLHttpRequest)();
  ...
  d3v3.text = function() {
    return d3v3.xhr.apply(d3v3, arguments).response(d3v3_text);
  };
  function d3v3_text(request) {
    return request.responseText;
  }
  d3v3.json = function(url, callback) {
    return d3v3.xhr(url, "application/json", callback).response(d3v3_json);
  };
  function d3v3_json(request) {
    return JSON.parse(request.responseText);
  }
  d3v3.html = function(url, callback) {
    return d3v3.xhr(url, "text/html", callback).response(d3v3_html);
  };
  ...
  d3v3.xml = function() {
    return d3v3.xhr.apply(d3v3, arguments).response(d3v3_xml);
  };
  function d3v3_xml(request) {
  ...
  d3v3.format = function(specifier) {
    var match = d3v3_format_re.exec(specifier), fill = match[1] || " ", align = match[2] || ">", sign = match[3] || "", basePrefix = match[4] || "", zfill = match[5], width = +match[6], comma = match[7], precision = match[8], type = match[9], scale = 1, suffix = "", integer = false;
    ...
    return function(value) {
      ...
      if (!zfill && comma) value = d3v3_format_group(value);
      var length = basePrefix.length + value.length + (zcomma ? 0 : negative.length), padding = length < width ? new Array(length = width - length + 1).join(fill) : "";
      if (zcomma) value = d3v3_format_group(padding + value);
      if (d3v3_format_decimalPoint) value.replace(".", d3v3_format_decimalPoint);
      negative += basePrefix;
      return (align === "<" ? negative + value + padding : align === ">" ? padding + negative + value : align === "^" ? padding.substring(0, length >>= 1) + negative + value + padding.substring(length) : negative + (zcomma ? value : padding + value)) + suffix;
    };
  };
  var d3v3_format_re = /(?:([^{])?([<>=^]))?([+\- ])?(#)?(0)?([0-9]+)?(,)?(\.[0-9]+)?([a-zA-Z%])?/;
  ...
  var d3v3_format_group = d3v3_identity;
  if (d3v3_format_grouping) {
    var d3v3_format_groupingLength = d3v3_format_grouping.length;
    d3v3_format_group = function(value) {
      var i = value.lastIndexOf("."), f = i >= 0 ? "." + value.substring(i + 1) : (i = value.length, 
      ""), t = [], j = 0, g = d3v3_format_grouping[0];
      while (i > 0 && g > 0) {
        t.push(value.substring(i -= g, i + g));
        g = d3v3_format_grouping[j = (j + 1) % d3v3_format_groupingLength];
      }
      return t.reverse().join(d3v3_format_thousandsSeparator || "") + f;
    };
  }
  ...
  d3v3.ease = function(name) {
    ...
    return d3v3_ease_clamp(m(t.apply(null, Array.prototype.slice.call(arguments, 1))));
  };
  ...
  d3v3.interpolateString = function(a, b) {
    ...
    d3v3_interpolate_number.lastIndex = 0;
    for (i = 0; m = d3v3_interpolate_number.exec(b); ++i) {
      ...
      s.push(null);
      s0 = d3v3_interpolate_number.lastIndex;
    }
    if (s0 < b.length) s.push(b.substring(s0));
    for (i = 0, n = q.length; (m = d3v3_interpolate_number.exec(a)) && i < n; ++i) {
      o = q[i];
      if (o.x == m[0]) {
        if (o.i) {
          if (s[o.i + 1] == null) {
  ...
  d3v3.interpolateHsl = function(a, b) {
    ...
    return function(t) {
      return d3v3_hsl_rgb(h0 + h1 * t, s0 + s1 * t, l0 + l1 * t) + "";
    };
  };
  ...
  d3v3.interpolateObject = function(a, b) {
    ...
    for (k in a) {
      if (k in b) {
        i[k] = d3v3_interpolateByName(k)(a[k], b[k]);
      } else {
        c[k] = a[k];
      }
    }
  ...
  var d3v3_interpolate_number = /[-+]?(?:\d+\.?\d*|\.?\d+)(?:[eE][-+]?\d+)?/g;
  function d3v3_interpolateByName(name) {
  ...
  d3v3.interpolators = [ d3v3.interpolateObject, function(a, b) {
    ...
    return (typeof b === "string" ? d3v3_rgb_names.has(b) || /^(#|rgb\(|hsl\()/.test(b) : b instanceof d3v3_Color) && d3v3.interpolateRgb(a, b);
  }, function(a, b) {
  ...
  d3v3.rgb = function(r, g, b) {
    return arguments.length === 1 ? r instanceof d3v3_Rgb ? d3v3_rgb(r.r, r.g, r.b) : d3v3_rgb_parse("" + r, d3v3_rgb, d3v3_hsl_rgb) : d3v3_rgb(~~r, ~~g, ~~b);
  };
  function d3v3_rgb(r, g, b) {
    return new d3v3_Rgb(r, g, b);
  }
  function d3v3_Rgb(r, g, b) {
    this.r = r;
    this.g = g;
  ...
  d3v3_rgbPrototype.brighter = function(k) {
    ...
    if (!r && !g && !b) return d3v3_rgb(i, i, i);
    if (r && r < i) r = i;
    if (g && g < i) g = i;
    if (b && b < i) b = i;
    return d3v3_rgb(Math.min(255, Math.floor(r / k)), Math.min(255, Math.floor(g / k)), Math.min(255, Math.floor(b / k)));
  };
  d3v3_rgbPrototype.darker = function(k) {
    k = Math.pow(.7, arguments.length ? k : 1);
    return d3v3_rgb(Math.floor(k * this.r), Math.floor(k * this.g), Math.floor(k * this.b));
  };
  ...
  function d3v3_rgb_parse(format, rgb, hsl) {
    ...
    if (name = d3v3_rgb_names.get(format)) return rgb(name.r, name.g, name.b);
  ...
  var d3v3_rgb_names = d3v3.map({
    ...
    lightgray: "#d3v3d3v3d3v3",
    lightgreen: "#90ee90",
    lightgrey: "#d3v3d3v3d3v3",
    lightpink: "#ffb6c1",
    lightsalmon: "#ffa07a",
    ...
    whitesmoke: "#f5f5f5",
    yellow: "#ffff00",
    yellowgreen: "#9acd3v32"
  });
  d3v3_rgb_names.forEach(function(key, value) {
    d3v3_rgb_names.set(key, d3v3_rgb_parse(value, d3v3_rgb, d3v3_hsl_rgb));
  });
  d3v3.hsl = function(h, s, l) {
    return arguments.length === 1 ? h instanceof d3v3_Hsl ? d3v3_hsl(h.h, h.s, h.l) : d3v3_rgb_parse("" + h, d3v3_rgb_hsl, d3v3_hsl) : d3v3_hsl(+h, +s, +l);
  };
  ...
  d3v3_hslPrototype.brighter = function(k) {
    k = Math.pow(.7, arguments.length ? k : 1);
    return d3v3_hsl(this.h, this.s, this.l / k);
  };
  d3v3_hslPrototype.darker = function(k) {
    k = Math.pow(.7, arguments.length ? k : 1);
    return d3v3_hsl(this.h, this.s, k * this.l);
  };
  ...
  function d3v3_hsl_rgb(h, s, l) {
    ...
    return d3v3_rgb(vv(h + 120), vv(h), vv(h - 120));
  }
  d3v3.hcl = function(h, c, l) {
    return arguments.length === 1 ? h instanceof d3v3_Hcl ? d3v3_hcl(h.h, h.c, h.l) : h instanceof d3v3_Lab ? d3v3_lab_hcl(h.l, h.a, h.b) : d3v3_lab_hcl((h = d3v3_rgb_lab((h = d3v3.rgb(h)).r, h.g, h.b)).l, h.a, h.b) : d3v3_hcl(+h, +c, +l);
  };
  function d3v3_hcl(h, c, l) {
    return new d3v3_Hcl(h, c, l);
  }
  function d3v3_Hcl(h, c, l) {
    this.h = h;
    this.c = c;
  ...
  d3v3_hclPrototype.brighter = function(k) {
    return d3v3_hcl(this.h, this.c, Math.min(100, this.l + d3v3_lab_K * (arguments.length ? k : 1)));
  };
  d3v3_hclPrototype.darker = function(k) {
    return d3v3_hcl(this.h, this.c, Math.max(0, this.l - d3v3_lab_K * (arguments.length ? k : 1)));
  };
  d3v3_hclPrototype.rgb = function() {
    return d3v3_hcl_lab(this.h, this.c, this.l).rgb();
  };
  function d3v3_hcl_lab(h, c, l) {
    return d3v3_lab(l, Math.cos(h *= d3v3_radians) * c, Math.sin(h) * c);
  }
  d3v3.lab = function(l, a, b) {
    return arguments.length === 1 ? l instanceof d3v3_Lab ? d3v3_lab(l.l, l.a, l.b) : l instanceof d3v3_Hcl ? d3v3_hcl_lab(l.l, l.c, l.h) : d3v3_rgb_lab((l = d3v3.rgb(l)).r, l.g, l.b) : d3v3_lab(+l, +a, +b);
  };
  function d3v3_lab(l, a, b) {
    return new d3v3_Lab(l, a, b);
  }
  function d3v3_Lab(l, a, b) {
    this.l = l;
    this.a = a;
  ...
  d3v3_labPrototype.brighter = function(k) {
    return d3v3_lab(Math.min(100, this.l + d3v3_lab_K * (arguments.length ? k : 1)), this.a, this.b);
  };
  d3v3_labPrototype.darker = function(k) {
    return d3v3_lab(Math.max(0, this.l - d3v3_lab_K * (arguments.length ? k : 1)), this.a, this.b);
  };
  ...
  function d3v3_lab_rgb(l, a, b) {
    ...
    return d3v3_rgb(d3v3_xyz_rgb(3.2404542 * x - 1.5371385 * y - .4985314 * z), d3v3_xyz_rgb(-.969266 * x + 1.8760108 * y + .041556 * z), d3v3_xyz_rgb(.0556434 * x - .2040259 * y + 1.0572252 * z));
  }
  function d3v3_lab_hcl(l, a, b) {
    return d3v3_hcl(Math.atan2(b, a) / π * 180, Math.sqrt(a * a + b * b), l);
  }
  ...
  function d3v3_selection(groups) {
    d3v3_arraySubclass(groups, d3v3_selectionPrototype);
    return groups;
  }
  ...
  }, d3v3_selectRoot = d3v3_document.documentElement, d3v3_selectMatcher = d3v3_selectRoot.matchesSelector || d3v3_selectRoot.webkitMatchesSelector || d3v3_selectRoot.mozMatchesSelector || d3v3_selectRoot.msMatchesSelector || d3v3_selectRoot.oMatchesSelector, d3v3_selectMatches = function(n, s) {
  ...
  var d3v3_selectionPrototype = [];
  d3v3.selection = function() {
    return d3v3_selectionRoot;
  };
  d3v3.selection.prototype = d3v3_selectionPrototype;
  d3v3_selectionPrototype.select = function(selector) {
    var subgroups = [], subgroup, subnode, group, node;
    if (typeof selector !== "function") selector = d3v3_selection_selector(selector);
  ...
  d3v3_selectionPrototype.selectAll = function(selector) {
    ...
    if (typeof selector !== "function") selector = d3v3_selection_selectorAll(selector);
    for (var j = -1, m = this.length; ++j < m; ) {
      for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
        if (node = group[i]) {
          subgroups.push(subgroup = d3v3_array(selector.call(node, node.__data__, i)));
  ...
  function d3v3_selection_classed(name, value) {
    name = name.trim().split(/\s+/).map(d3v3_selection_classedName);
    var n = name.length;
  ...
  function d3v3_selection_classedName(name) {
    ...
    return function(node, value) {
      ...
      if (value) {
        re.lastIndex = 0;
        if (!re.test(cv)) {
          cv = d3v3_collapse(cv + " " + name);
          if (cb) c.baseVal = cv; else node.className = cv;
        }
      } else if (cv) {
        cv = d3v3_collapse(cv.replace(re, " "));
        if (cb) c.baseVal = cv; else node.className = cv;
      }
    };
  }
  ...
  d3v3_selectionPrototype.append = function(name) {
    name = d3v3.ns.qualify(name);
    function append() {
      return this.appendChild(d3v3_document.createElementNS(this.namespaceURI, name));
    }
    function appendNS() {
      return this.appendChild(d3v3_document.createElementNS(name.space, name.local));
    }
  ...
  d3v3_selectionPrototype.insert = function(name, before) {
    name = d3v3.ns.qualify(name);
    function insert() {
      return this.insertBefore(d3v3_document.createElementNS(this.namespaceURI, name), d3v3_select(before, this));
    }
    function insertNS() {
      return this.insertBefore(d3v3_document.createElementNS(name.space, name.local), d3v3_select(before, this));
    }
  ...
  d3v3_selectionPrototype.data = function(value, key) {
    ...
    function bind(group, groupData) {
      ...
      if (key) {
        var nodeByKeyValue = new d3v3_Map(), dataByKeyValue = new d3v3_Map(), keyValues = [], keyValue;
        for (i = -1; ++i < n; ) {
  ...
  function d3v3_selection_on(type, listener, capture) {
    ...
    function onAdd() {
      var node = this, args = d3v3_array(arguments);
      onRemove.call(this);
  ...
  d3v3_selectionPrototype.transition = function() {
    var id = d3v3_transitionInheritId || ++d3v3_transitionId, subgroups = [], subgroup, node, transition = Object.create(d3v3_transitionInherit);
    transition.time = Date.now();
    for (var j = -1, m = this.length; ++j < m; ) {
      subgroups.push(subgroup = []);
      for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
        if (node = group[i]) d3v3_transitionNode(node, i, id, transition);
        subgroup.push(node);
      }
    }
    return d3v3_transition(subgroups, id);
  };
  var d3v3_selectionRoot = d3v3_selection([ [ d3v3_document ] ]);
  d3v3_selectionRoot[0].parentNode = d3v3_selectRoot;
  d3v3.select = function(selector) {
    return typeof selector === "string" ? d3v3_selectionRoot.select(selector) : d3v3_selection([ [ selector ] ]);
  };
  d3v3.selectAll = function(selector) {
    return typeof selector === "string" ? d3v3_selectionRoot.selectAll(selector) : d3v3_selection([ d3v3_array(selector) ]);
  };
  function d3v3_selection_enter(selection) {
    d3v3_arraySubclass(selection, d3v3_selection_enterPrototype);
    return selection;
  }
  ...
  d3v3_selection_enterPrototype.insert = d3v3_selectionPrototype.insert;
  ...
  function d3v3_transition(groups, id) {
    d3v3_arraySubclass(groups, d3v3_transitionPrototype);
    groups.id = id;
    return groups;
  }
  ...
  d3v3.transition = function(selection) {
    return arguments.length ? d3v3_transitionInheritId ? selection.transition() : selection : d3v3_selectionRoot.transition();
  };
  d3v3.transition.prototype = d3v3_transitionPrototype;
  function d3v3_transitionNode(node, i, id, inherit) {
    ...
    for (var j = -1, m = this.length; ++j < m; ) {
      ...
      for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
        if ((node = group[i]) && (subnode = selector.call(node, node.__data__, i))) {
          ...
          d3v3_transitionNode(subnode, i, id, node.__transition__[id]);
      ...
      for (var group = this[j], i = -1, n = group.length; ++i < n; ) {
        if (node = group[i]) {
          ...
          for (var k = -1, o = subnodes.length; ++k < o; ) {
            d3v3_transitionNode(subnode = subnodes[k], k, id, transition);
            subgroup.push(subnode);
          }
        }
      }
  ...
  d3v3_transitionPrototype.filter = function(filter) {
    ...
    return d3v3_transition(subgroups, this.id, this.time).ease(this.ease());
  };
  d3v3_transitionPrototype.attr = function(nameNS, value) {
    ...
    var interpolate = d3v3_interpolateByName(nameNS), name = d3v3.ns.qualify(nameNS);
    function attrNull() {
      this.removeAttribute(name);
    }
    function attrNullNS() {
    ...
    return d3v3_transition_tween(this, "attr." + nameNS, value, function(b) {
      function attrString() {
  ...
  d3v3_transitionPrototype.style = function(name, value, priority) {
    ...
    var interpolate = d3v3_interpolateByName(name);
    function styleNull() {
      this.style.removeProperty(name);
    }
    return d3v3_transition_tween(this, "style." + name, value, function(b) {
  ...
  d3v3_transitionPrototype.each = function(type, listener) {
    ...
    if (arguments.length < 2) {
      ...
      d3v3_selection_each(this, function(node, i, j) {
        d3v3_transitionInherit = node.__transition__[id];
  ...
  d3v3_transitionPrototype.transition = function() {
    var id0 = this.id, id1 = ++d3v3_transitionId, subgroups = [], subgroup, group, node, transition;
    for (var j = 0, m = this.length; j < m; j++) {
      subgroups.push(subgroup = []);
      for (var group = this[j], i = 0, n = group.length; i < n; i++) {
        if (node = group[i]) {
          ...
          d3v3_transitionNode(node, i, id1, transition);
        }
        subgroup.push(node);
      }
    }
    return d3v3_transition(subgroups, id1);
  };
  ...
  var d3v3_timer_id = 0, d3v3_timer_byId = {}, d3v3_timer_queue = null, d3v3_timer_interval, d3v3_timer_timeout;
  d3v3.timer = function(callback, delay, then) {
    if (arguments.length < 3) {
      if (arguments.length < 2) delay = 0; else if (!isFinite(delay)) return;
      then = Date.now();
    }
    var timer = d3v3_timer_byId[callback.id];
    if (timer && timer.callback === callback) {
      timer.then = then;
      timer.delay = delay;
    } else d3v3_timer_byId[callback.id = ++d3v3_timer_id] = d3v3_timer_queue = {
      callback: callback,
      then: then,
      delay: delay,
      next: d3v3_timer_queue
    };
  ...
  function d3v3_timer_step() {
    var elapsed, now = Date.now(), t1 = d3v3_timer_queue;
    while (t1) {
      elapsed = now - t1.then;
      if (elapsed >= t1.delay) t1.flush = t1.callback(elapsed);
      t1 = t1.next;
    }
    var delay = d3v3_timer_flush() - now;
    if (delay > 24) {
      if (isFinite(delay)) {
  ...
  function d3v3_timer_flush() {
    var t0 = null, t1 = d3v3_timer_queue, then = Infinity;
    while (t1) {
      if (t1.flush) {
        delete d3v3_timer_byId[t1.callback.id];
        t1 = t0 ? t0.next = t1.next : d3v3_timer_queue = t1.next;
      } else {
  ...
  var d3v3_timer_frame = d3v3_window.requestAnimationFrame || d3v3_window.webkitRequestAnimationFrame || d3v3_window.mozRequestAnimationFrame || d3v3_window.oRequestAnimationFrame || d3v3_window.msRequestAnimationFrame || function(callback) {
  ...
  function d3v3_mousePoint(container, e) {
    ...
    if (svg.createSVGPoint) {
      ...
      if (d3v3_mouse_bug44083 < 0 && (d3v3_window.scrollX || d3v3_window.scrollY)) {
        svg = d3v3.select(d3v3_document.body).append("svg").style("position", "absolute").style("top", 0).style("left", 0);
        var ctm = svg[0][0].getScreenCTM();
        d3v3_mouse_bug44083 = !(ctm.f || ctm.e);
        svg.remove();
      }
      if (d3v3_mouse_bug44083) {
        point.x = e.pageX;
        point.y = e.pageY;
      } else {
        point.x = e.clientX;
  ...
  function d3v3_scale_niceDefault() {
    return Math;
  }
  d3v3.scale.linear = function() {
    return d3v3_scale_linear([ 0, 1 ], [ 0, 1 ], d3v3.interpolate, false);
  };
  function d3v3_scale_linear(domain, range, interpolate, clamp) {
    ...
    scale.tickFormat = function(m) {
      return d3v3_scale_linearTickFormat(domain, m);
    };
    scale.nice = function() {
      d3v3_scale_nice(domain, d3v3_scale_linearNice);
      return rescale();
    };
    scale.copy = function() {
  ...
  function d3v3_scale_linearNice(dx) {
  ...
  function d3v3_scale_linearTickFormat(domain, m) {
    return d3v3.format(",." + Math.max(0, -Math.floor(Math.log(d3v3_scale_linearTickRange(domain, m)[2]) / Math.LN10 + .01)) + "f");
  }
  ...
  d3v3.scale.log = function() {
    return d3v3_scale_log(d3v3.scale.linear(), d3v3_scale_logp);
  };
  function d3v3_scale_log(linear, log) {
    var pow = log.pow;
    function scale(x) {
      return linear(log(x));
    }
    ...
    scale.domain = function(x) {
      ...
      log = x[0] < 0 ? d3v3_scale_logn : d3v3_scale_logp;
      pow = log.pow;
      linear.domain(x.map(log));
      return scale;
    };
    scale.nice = function() {
      linear.domain(d3v3_scale_nice(linear.domain(), d3v3_scale_niceDefault));
      return scale;
    };
    scale.ticks = function() {
      var extent = d3v3_scaleExtent(linear.domain()), ticks = [];
      if (extent.every(isFinite)) {
        var i = Math.floor(extent[0]), j = Math.ceil(extent[1]), u = pow(extent[0]), v = pow(extent[1]);
        if (log === d3v3_scale_logn) {
          ticks.push(pow(i));
    ...
    scale.tickFormat = function(n, format) {
      if (arguments.length < 2) format = d3v3_scale_logFormat;
      if (!arguments.length) return format;
      var k = Math.max(.1, n / scale.ticks().length), f = log === d3v3_scale_logn ? (e = -1e-12, 
    ...
    scale.copy = function() {
      return d3v3_scale_log(linear.copy(), log);
    };
    return d3v3_scale_linearRebind(scale, linear);
  }
  var d3v3_scale_logFormat = d3v3.format(".0e");
  function d3v3_scale_logp(x) {
    return Math.log(x < 0 ? 0 : x) / Math.LN10;
  }
  function d3v3_scale_logn(x) {
    return -Math.log(x > 0 ? 0 : -x) / Math.LN10;
  }
  d3v3_scale_logp.pow = function(x) {
    return Math.pow(10, x);
  };
  d3v3_scale_logn.pow = function(x) {
    return -Math.pow(10, -x);
  };
  d3v3.scale.pow = function() {
    return d3v3_scale_pow(d3v3.scale.linear(), 1);
  };
  function d3v3_scale_pow(linear, exponent) {
    ...
    scale.ticks = function(m) {
      return d3v3_scale_linearTicks(scale.domain(), m);
    };
    scale.tickFormat = function(m) {
      return d3v3_scale_linearTickFormat(scale.domain(), m);
    };
    scale.nice = function() {
      return scale.domain(d3v3_scale_nice(scale.domain(), d3v3_scale_linearNice));
    };
    ...
    scale.copy = function() {
      return d3v3_scale_pow(linear.copy(), exponent);
    };
    return d3v3_scale_linearRebind(scale, linear);
  }
  ...
  var d3v3_category10 = [ "#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf" ];
  var d3v3_category20 = [ "#1f77b4", "#aec7e8", "#ff7f0e", "#ffbb78", "#2ca02c", "#98df8a", "#d62728", "#ff9896", "#9467bd", "#c5b0d5", "#8c564b", "#c49c94", "#e377c2", "#f7b6d2", "#7f7f7f", "#c7c7c7", "#bcbd22", "#dbdb8d", "#17becf", "#9edae5" ];
  var d3v3_category20b = [ "#393b79", "#5254a3", "#6b6ecf", "#9c9ede", "#637939", "#8ca252", "#b5cf6b", "#cedb9c", "#8c6d3v31", "#bd9e39", "#e7ba52", "#e7cb94", "#843c39", "#ad494a", "#d6616b", "#e7969c", "#7b4173", "#a55194", "#ce6dbd", "#de9ed6" ];
  var d3v3_category20c = [ "#3182bd", "#6baed6", "#9ecae1", "#c6dbef", "#e6550d", "#fd8d3v3c", "#fdae6b", "#fdd0a2", "#31a354", "#74c476", "#a1d99b", "#c7e9c0", "#756bb1", "#9e9ac8", "#bcbddc", "#dadaeb", "#636363", "#969696", "#bdbdbd", "#d9d9d9" ];
  ...
  d3v3.svg.arc = function() {
    var innerRadius = d3v3_svg_arcInnerRadius, outerRadius = d3v3_svg_arcOuterRadius, startAngle = d3v3_svg_arcStartAngle, endAngle = d3v3_svg_arcEndAngle;
    function arc() {
      var r0 = innerRadius.apply(this, arguments), r1 = outerRadius.apply(this, arguments), a0 = startAngle.apply(this, arguments) + d3v3_svg_arcOffset, a1 = endAngle.apply(this, arguments) + d3v3_svg_arcOffset, da = (a1 < a0 && (da = a0, 
      a0 = a1, a1 = da), a1 - a0), df = da < π ? "0" : "1", c0 = Math.cos(a0), s0 = Math.sin(a0), c1 = Math.cos(a1), s1 = Math.sin(a1);
      return da >= d3v3_svg_arcMax ? r0 ? "M0," + r1 + "A" + r1 + "," + r1 + " 0 1,1 0," + -r1 + "A" + r1 + "," + r1 + " 0 1,1 0," + r1 + "M0," + r0 + "A" + r0 + "," + r0 + " 0 1,0 0," + -r0 + "A" + r0 + "," + r0 + " 0 1,0 0," + r0 + "Z" : "M0," + r1 + "A" + r1 + "," + r1 + " 0 1,1 0," + -r1 + "A" + r1 + "," + r1 + " 0 1,1 0," + r1 + "Z" : r0 ? "M" + r1 * c0 + "," + r1 * s0 + "A" + r1 + "," + r1 + " 0 " + df + ",1 " + r1 * c1 + "," + r1 * s1 + "L" + r0 * c1 + "," + r0 * s1 + "A" + r0 + "," + r0 + " 0 " + df + ",0 " + r0 * c0 + "," + r0 * s0 + "Z" : "M" + r1 * c0 + "," + r1 * s0 + "A" + r1 + "," + r1 + " 0 " + df + ",1 " + r1 * c1 + "," + r1 * s1 + "L0,0" + "Z";
    }
    ...
    arc.centroid = function() {
      var r = (innerRadius.apply(this, arguments) + outerRadius.apply(this, arguments)) / 2, a = (startAngle.apply(this, arguments) + endAngle.apply(this, arguments)) / 2 + d3v3_svg_arcOffset;
      return [ Math.cos(a) * r, Math.sin(a) * r ];
    };
    return arc;
  };
  var d3v3_svg_arcOffset = -π / 2, d3v3_svg_arcMax = 2 * π - 1e-6;
  ...
  function d3v3_svg_line(projection) {
    var x = d3v3_svg_lineX, y = d3v3_svg_lineY, defined = d3v3_true, interpolate = d3v3_svg_lineLinear, interpolateKey = interpolate.key, tension = .7;
    function line(data) {
  ...
  function d3v3_svg_lineX(d) {
    return d[0];
  }
  function d3v3_svg_lineY(d) {
    return d[1];
  }
  ...
  function d3v3_svg_lineCardinalOpen(points, tension) {
    return points.length < 4 ? d3v3_svg_lineLinear(points) : points[1] + d3v3_svg_lineHermite(points.slice(1, points.length - 1), d3v3_svg_lineCardinalTangents(points, tension));
  }
  ...
  function d3v3_svg_area(projection) {
    var x0 = d3v3_svg_lineX, x1 = d3v3_svg_lineX, y0 = 0, y1 = d3v3_svg_lineY, defined = d3v3_true, interpolate = d3v3_svg_lineLinear, interpolateKey = interpolate.key, interpolateReverse = interpolate, L = "L", tension = .7;
    function area(data) {
  ...
  d3v3.svg.chord = function() {
    ...
    function subgroup(self, f, d, i) {
      var subgroup = f.call(self, d, i), r = radius.call(self, subgroup, i), a0 = startAngle.call(self, subgroup, i) + d3v3_svg_arcOffset, a1 = endAngle.call(self, subgroup, i) + d3v3_svg_arcOffset;
      return {
        r: r,
        a0: a0,
        a1: a1,
  ...
  function d3v3_svg_diagonalRadialProjection(projection) {
    return function() {
      var d = projection.apply(this, arguments), r = d[0], a = d[1] + d3v3_svg_arcOffset;
  ...
  d3v3.svg.axis = function() {
    var scale = d3v3.scale.linear(), orient = d3v3_svg_axisDefaultOrient, tickMajorSize = 6, tickMinorSize = 6, tickEndSize = 6, tickPadding = 3, tickArguments_ = [ 10 ], tickValues = null, tickFormat_, tickSubdivide = 0;
    function axis(g) {
      g.each(function() {
        var g = d3v3.select(this);
        var ticks = tickValues == null ? scale.ticks ? scale.ticks.apply(scale, tickArguments_) : scale.domain() : tickValues, tickFormat = tickFormat_ == null ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments_) : String : tickFormat_;
        var subticks = d3v3_svg_axisSubdivide(scale, ticks, tickSubdivide), subtick = g.selectAll(".tick.minor").data(subticks, String), subtickEnter = subtick.enter().insert("line", ".tick").attr("class", "tick minor").style("opacity", 1e-6), subtickExit = d3v3.transition(subtick.exit()).style("opacity", 1e-6).remove(), subtickUpdate = d3v3.transition(subtick).style("opacity", 1);
        var tick = g.selectAll(".tick.major").data(ticks, String), tickEnter = tick.enter().insert("g", "path").attr("class", "tick major").style("opacity", 1e-6), tickExit = d3v3.transition(tick.exit()).style("opacity", 1e-6).remove(), tickUpdate = d3v3.transition(tick).style("opacity", 1), tickTransform;
        var range = d3v3_scaleRange(scale), path = g.selectAll(".domain").data([ 0 ]), pathUpdate = (path.enter().append("path").attr("class", "domain"), 
  ...
  function d3v3_svg_axisX(selection, x) {
    selection.attr("transform", function(d) {
      return "translate(" + x(d) + ",0)";
    });
  }
  function d3v3_svg_axisY(selection, y) {
    selection.attr("transform", function(d) {
      return "translate(0," + y(d) + ")";
    });
  }
  function d3v3_svg_axisSubdivide(scale, ticks, m) {
    subticks = [];
    if (m && ticks.length > 1) {
      var extent = d3v3_scaleExtent(scale.domain()), subticks, i = -1, n = ticks.length, d = (ticks[1] - ticks[0]) / ++m, j, v;
      while (++i < n) {
        for (j = m; --j > 0; ) {
  ...
  d3v3.svg.brush = function() {
    var event = d3v3_eventDispatch(brush, "brushstart", "brush", "brushend"), x = null, y = null, resizes = d3v3_svg_brushResizes[0], extent = [ [ 0, 0 ], [ 0, 0 ] ], extentDomain;
    function brush(g) {
      g.each(function() {
    ...
    function brushstart() {
      ...
      var w = d3v3.select(d3v3_window).on("mousemove.brush", brushmove).on("mouseup.brush", brushend).on("touchmove.brush", brushmove).on("touchend.brush", brushend).on("keydown.brush", keydown).on("keyup.brush", keyup);
      if (dragging) {
      ...
      function move1(point, scale, i) {
        var range = d3v3_scaleRange(scale), r0 = range[0], r1 = range[1], position = origin[i], size = extent[1][i] - extent[0][i], min, max;
        if (dragging) {
          r0 -= position;
          r1 -= size + position;
        }
  ...
  d3v3.behavior.drag = function() {
    var event = d3v3_eventDispatch(drag, "drag", "dragstart", "dragend"), origin = null;
    function drag() {
    ...
    function mousedown() {
      ...
      var w = d3v3.select(d3v3_window).on(touchId != null ? "touchmove.drag-" + touchId : "mousemove.drag", dragmove).on(touchId != null ? "touchend.drag-" + touchId : "mouseup.drag", dragend, true);
      if (origin) {
      ...
      if (touchId == null) d3v3_eventCancel();
      event_({
        type: "dragstart"
      });
      function point() {
  ...
  d3v3.behavior.zoom = function() {
    var translate = [ 0, 0 ], translate0, scale = 1, scale0, scaleExtent = d3v3_behavior_zoomInfinity, event = d3v3_eventDispatch(zoom, "zoom"), x0, x1, y0, y1, touchtime;
    function zoom() {
      this.on("mousedown.zoom", mousedown).on("mousemove.zoom", mousemove).on(d3v3_behavior_zoomWheel + ".zoom", mousewheel).on("dblclick.zoom", dblclick).on("touchstart.zoom", touchstart).on("touchmove.zoom", touchmove).on("touchend.zoom", touchstart);
    }
    ...
    zoom.scaleExtent = function(x) {
      ...
      scaleExtent = x == null ? d3v3_behavior_zoomInfinity : x.map(Number);
      return zoom;
    };
    zoom.x = function(z) {
      if (!arguments.length) return x1;
    ...
    function mousedown() {
      var target = this, event_ = event.of(target, arguments), eventTarget = d3v3.event.target, moved = 0, w = d3v3.select(d3v3_window).on("mousemove.zoom", mousemove).on("mouseup.zoom", mouseup), l = location(d3v3.mouse(target));
      d3v3_window.focus();
      d3v3_eventCancel();
      function mousemove() {
        moved = 1;
    ...
    function mousewheel() {
      ...
      scaleTo(Math.pow(2, d3v3_behavior_zoomDelta() * .002) * scale);
  ...
  var d3v3_behavior_zoomDelta, d3v3_behavior_zoomWheel = "onwheel" in document ? (d3v3_behavior_zoomDelta = function() {
    return -d3v3.event.deltaY * (d3v3.event.deltaMode ? 120 : 1);
  }, "wheel") : "onmousewheel" in document ? (d3v3_behavior_zoomDelta = function() {
  ...
  d3v3.layout.force = function() {
    var force = {}, event = d3v3.dispatch("start", "tick", "end"), size = [ 1, 1 ], drag, alpha, friction = .9, linkDistance = d3v3_layout_forceLinkDistance, linkStrength = d3v3_layout_forceLinkStrength, charge = -30, gravity = .1, theta = .8, nodes = [], links = [], distances, strengths, charges;
  ...
  var d3v3_layout_forceLinkDistance = 20, d3v3_layout_forceLinkStrength = 1;
  ...
  d3v3.layout.pie = function() {
    var value = Number, sort = d3v3_layout_pieSortByValue, startAngle = 0, endAngle = 2 * π;
    function pie(data) {
  ...
  d3v3.layout.pack = function() {
    var hierarchy = d3v3.layout.hierarchy().sort(d3v3_layout_packSort), padding = 0, size = [ 1, 1 ];
    function pack(d, i) {
      var nodes = hierarchy.call(this, d, i), root = nodes[0];
      root.x = 0;
      root.y = 0;
      d3v3_layout_treeVisitAfter(root, function(d) {
        d.r = Math.sqrt(d.value);
      });
      d3v3_layout_treeVisitAfter(root, d3v3_layout_packSiblings);
      ...
      d3v3_layout_packTransform(root, w / 2, h / 2, 1 / k);
      return nodes;
    }
  ...
  d3v3.layout.cluster = function() {
    var hierarchy = d3v3.layout.hierarchy().sort(null).value(null), separation = d3v3_layout_treeSeparation, size = [ 1, 1 ];
    function cluster(d, i) {
      var nodes = hierarchy.call(this, d, i), root = nodes[0], previousNode, x = 0;
      d3v3_layout_treeVisitAfter(root, function(node) {
        var children = node.children;
  ...
  d3v3.layout.tree = function() {
    ...
    function tree(d, i) {
      ...
      function apportion(node, previousSibling, ancestor) {
        if (previousSibling) {
          ...
          while (vim = d3v3_layout_treeRight(vim), vip = d3v3_layout_treeLeft(vip), vim && vip) {
            ...
            if (shift > 0) {
              d3v3_layout_treeMove(d3v3_layout_treeAncestor(vim, node, ancestor), node, shift);
              sip += shift;
              sop += shift;
            }
            sim += vim._tree.mod;
      ...
      d3v3_layout_treeVisitAfter(root, function(node, previousSibling) {
        node._tree = {
          ancestor: node,
          prelim: 0,
          mod: 0,
      ...
      var left = d3v3_layout_treeSearch(root, d3v3_layout_treeLeftmost), right = d3v3_layout_treeSearch(root, d3v3_layout_treeRightmost), deep = d3v3_layout_treeSearch(root, d3v3_layout_treeDeepest), x0 = left.x - separation(left, right) / 2, x1 = right.x + separation(right, left) / 2, y1 = deep.depth || 1;
      d3v3_layout_treeVisitAfter(root, function(node) {
        node.x = (node.x - x0) / (x1 - x0) * size[0];
  ...
  function d3v3_layout_treeLeft(node) {
    var children = node.children;
    return children && children.length ? children[0] : node._tree.thread;
  }
  function d3v3_layout_treeRight(node) {
    var children = node.children, n;
    return children && (n = children.length) ? children[n - 1] : node._tree.thread;
  }
  function d3v3_layout_treeSearch(node, compare) {
    var children = node.children;
    if (children && (n = children.length)) {
      var child, n, i = -1;
      while (++i < n) {
        if (compare(child = d3v3_layout_treeSearch(children[i], compare), node) > 0) {
          node = child;
        }
      }
    }
  ...
  function d3v3_layout_treeRightmost(a, b) {
    return a.x - b.x;
  }
  function d3v3_layout_treeLeftmost(a, b) {
    return b.x - a.x;
  }
  function d3v3_layout_treeDeepest(a, b) {
    return a.depth - b.depth;
  }
  function d3v3_layout_treeVisitAfter(node, callback) {
  ...
  function d3v3_layout_treeShift(node) {
  ...
  function d3v3_layout_treeMove(ancestor, node, shift) {
  ...
  function d3v3_layout_treeAncestor(vim, node, ancestor) {
  ...
  function d3v3_dsv(delimiter, mimeType) {
  ...
  d3v3.csv = d3v3_dsv(",", "text/csv");
  d3v3.tsv = d3v3_dsv("	", "text/tab-separated-values");
  d3v3.geo = {};
  d3v3.geo.stream = function(object, listener) {
    if (d3v3_geo_streamObjectType.hasOwnProperty(object.type)) {
  ...
  function d3v3_geo_streamGeometry(geometry, listener) {
    if (d3v3_geo_streamGeometryType.hasOwnProperty(geometry.type)) {
  ...
  function d3v3_geo_resample(project) {
    ...
    function resample(stream) {
      ...
      function linePoint(λ, φ) {
        var c = d3v3_geo_cartesian([ λ, φ ]), p = project(λ, φ);
  ...
  function d3v3_geo_albers(φ0, φ1) {
  ...
  (d3v3.geo.albers = function() {
    var φ0 = 29.5 * d3v3_radians, φ1 = 45.5 * d3v3_radians, m = d3v3_geo_projectionMutator(d3v3_geo_albers), p = m(φ0, φ1);
    p.parallels = function(_) {
      if (!arguments.length) return [ φ0 * d3v3_degrees, φ1 * d3v3_degrees ];
      return m(φ0 = _[0] * d3v3_radians, φ1 = _[1] * d3v3_radians);
    };
    return p.rotate([ 98, 0 ]).center([ 0, 38 ]).scale(1e3);
  }).raw = d3v3_geo_albers;
  var d3v3_geo_azimuthalEqualArea = d3v3_geo_azimuthal(function(cosλcosφ) {
  ...
  var d3v3_geo_azimuthalEquidistant = d3v3_geo_azimuthal(function(cosλcosφ) {
  ...
  d3v3.geo.bounds = d3v3_geo_bounds(d3v3_identity);
  function d3v3_geo_bounds(projectStream) {
    var x0, y0, x1, y1;
    var bound = {
      point: boundPoint,
      lineStart: d3v3_noop,
    ...
    function boundPolygonLineEnd() {
      bound.point = bound.lineEnd = d3v3_noop;
    }
  ...
  d3v3.geo.centroid = function(object) {
    d3v3_geo_centroidDimension = d3v3_geo_centroidW = d3v3_geo_centroidX = d3v3_geo_centroidY = d3v3_geo_centroidZ = 0;
    d3v3.geo.stream(object, d3v3_geo_centroid);
    var m;
    if (d3v3_geo_centroidW && Math.abs(m = Math.sqrt(d3v3_geo_centroidX * d3v3_geo_centroidX + d3v3_geo_centroidY * d3v3_geo_centroidY + d3v3_geo_centroidZ * d3v3_geo_centroidZ)) > ε) {
      return [ Math.atan2(d3v3_geo_centroidY, d3v3_geo_centroidX) * d3v3_degrees, Math.asin(Math.max(-1, Math.min(1, d3v3_geo_centroidZ / m))) * d3v3_degrees ];
    }
  };
  var d3v3_geo_centroidDimension, d3v3_geo_centroidW, d3v3_geo_centroidX, d3v3_geo_centroidY, d3v3_geo_centroidZ;
  var d3v3_geo_centroid = {
    sphere: function() {
      if (d3v3_geo_centroidDimension < 2) {
        d3v3_geo_centroidDimension = 2;
        d3v3_geo_centroidW = d3v3_geo_centroidX = d3v3_geo_centroidY = d3v3_geo_centroidZ = 0;
      }
    },
  ...
  function d3v3_geo_centroidPoint(λ, φ) {
    if (d3v3_geo_centroidDimension) return;
    ++d3v3_geo_centroidW;
    λ *= d3v3_radians;
    var cosφ = Math.cos(φ *= d3v3_radians);
    d3v3_geo_centroidX += (cosφ * Math.cos(λ) - d3v3_geo_centroidX) / d3v3_geo_centroidW;
    d3v3_geo_centroidY += (cosφ * Math.sin(λ) - d3v3_geo_centroidY) / d3v3_geo_centroidW;
    d3v3_geo_centroidZ += (Math.sin(φ) - d3v3_geo_centroidZ) / d3v3_geo_centroidW;
  }
  function d3v3_geo_centroidRingStart() {
    var λ00, φ00;
    d3v3_geo_centroidDimension = 1;
    d3v3_geo_centroidLineStart();
    d3v3_geo_centroidDimension = 2;
    var linePoint = d3v3_geo_centroid.point;
    d3v3_geo_centroid.point = function(λ, φ) {
      linePoint(λ00 = λ, φ00 = φ);
    };
    d3v3_geo_centroid.lineEnd = function() {
      d3v3_geo_centroid.point(λ00, φ00);
      d3v3_geo_centroidLineEnd();
      d3v3_geo_centroid.lineEnd = d3v3_geo_centroidLineEnd;
    };
  }
  function d3v3_geo_centroidLineStart() {
    var x0, y0, z0;
    if (d3v3_geo_centroidDimension > 1) return;
    if (d3v3_geo_centroidDimension < 1) {
      d3v3_geo_centroidDimension = 1;
      d3v3_geo_centroidW = d3v3_geo_centroidX = d3v3_geo_centroidY = d3v3_geo_centroidZ = 0;
    }
    d3v3_geo_centroid.point = function(λ, φ) {
    ...
    function nextPoint(λ, φ) {
      λ *= d3v3_radians;
      var cosφ = Math.cos(φ *= d3v3_radians), x = cosφ * Math.cos(λ), y = cosφ * Math.sin(λ), z = Math.sin(φ), w = Math.atan2(Math.sqrt((w = y0 * z - z0 * y) * w + (w = z0 * x - x0 * z) * w + (w = x0 * y - y0 * x) * w), x0 * x + y0 * y + z0 * z);
      d3v3_geo_centroidW += w;
      d3v3_geo_centroidX += w * (x0 + (x0 = x));
      d3v3_geo_centroidY += w * (y0 + (y0 = y));
      d3v3_geo_centroidZ += w * (z0 + (z0 = z));
    }
  }
  ...
  function d3v3_geo_circleInterpolate(radians, precision) {
  ...
  function d3v3_geo_clip(pointVisible, clipLine, interpolate) {
    return function(listener) {
      ...
      var clip = {
        ...
        polygonEnd: function() {
          ...
          if (segments.length) {
            d3v3_geo_clipPolygon(segments, interpolate, listener);
      ...
      var buffer = d3v3_geo_clipBufferListener(), ringListener = clipLine(buffer), ring;
      ...
      function ringEnd() {
        ...
        if (!n) {
          invisible = true;
          invisibleArea += d3v3_geo_clipAreaRing(ring, -1);
          ring = null;
          return;
        }
        ring = null;
        if (clean & 1) {
          ...
          visibleArea += d3v3_geo_clipAreaRing(segment, 1);
  ...
  function d3v3_geo_clipPolygon(segments, interpolate, listener) {
    var subject = [], clip = [];
    ...
    clip.sort(d3v3_geo_clipSort);
    d3v3_geo_clipLinkCircular(subject);
    d3v3_geo_clipLinkCircular(clip);
  ...
  function d3v3_geo_clipLinkCircular(array) {
  ...
  function d3v3_geo_clipAreaRing(ring, invisible) {
  ...
  var d3v3_geo_clipAntimeridian = d3v3_geo_clip(d3v3_true, d3v3_geo_clipAntimeridianLine, d3v3_geo_clipAntimeridianInterpolate);
    ...
    return {
      ...
      point: function(λ1, φ1) {
        ...
        if (Math.abs(dλ - π) < ε) {
          ...
          φ0 = d3v3_geo_clipAntimeridianIntersect(λ0, φ0, λ1, φ1);
          listener.point(sλ0, φ0);
          listener.lineEnd();
          listener.lineStart();
          listener.point(sλ1, φ0);
  ...
  function d3v3_geo_clipAntimeridianIntersect(λ0, φ0, λ1, φ1) {
  ...
  function d3v3_geo_clipCircle(degrees) {
    var radians = degrees * d3v3_radians, cr = Math.cos(radians), interpolate = d3v3_geo_circleInterpolate(radians, 6 * d3v3_radians);
    return d3v3_geo_clip(visible, clipLine, interpolate);
    ...
    function clipLine(listener) {
      ...
      return {
        ...
        point: function(λ, φ) {
          ...
          if (v && (!point0 || !d3v3_geo_sphericalEqual(point0, point1))) listener.point(point1[0], point1[1]);
          point0 = point1;
        },
        lineEnd: function() {
    ...
    function intersect(a, b) {
      var pa = d3v3_geo_cartesian(a, 0), pb = d3v3_geo_cartesian(b, 0);
      ...
      var u = n1xn2, w = d3v3_geo_cartesianDot(A, u), uu = d3v3_geo_cartesianDot(u, u), t = Math.sqrt(w * w - uu * (d3v3_geo_cartesianDot(A, A) - 1)), q = d3v3_geo_cartesianScale(u, (-w - t) / uu);
      d3v3_geo_cartesianAdd(q, A);
      return d3v3_geo_spherical(q);
    }
  }
  ...
  function d3v3_geo_equirectangular(λ, φ) {
    return [ λ, φ ];
  }
  (d3v3.geo.equirectangular = function() {
    return d3v3_geo_projection(d3v3_geo_equirectangular).scale(250 / π);
  }).raw = d3v3_geo_equirectangular.invert = d3v3_geo_equirectangular;
  var d3v3_geo_gnomonic = d3v3_geo_azimuthal(function(cosλcosφ) {
    return 1 / cosλcosφ;
  }, Math.atan);
  ...
  d3v3.geo.graticule = function() {
    ...
    graticule.precision = function(_) {
      if (!arguments.length) return precision;
      precision = +_;
      x = d3v3_geo_graticuleX(y0, y1, precision);
  ...
  function d3v3_geo_interpolate(x0, y0, x1, y1) {
    ...
    function interpolate(t) {
      ...
      return [ Math.atan2(y, x) / d3v3_radians, Math.atan2(z, Math.sqrt(x * x + y * y)) / d3v3_radians ];
    }
    interpolate.distance = d;
    return interpolate;
  }
  d3v3.geo.greatArc = function() {
    var source = d3v3_source, source_, target = d3v3_target, target_, precision = 6 * d3v3_radians, interpolate;
    function greatArc() {
    ...
    greatArc.precision = function(_) {
      if (!arguments.length) return precision / d3v3_radians;
      precision = _ * d3v3_radians;
      return greatArc;
    };
    return greatArc;
  };
  function d3v3_geo_mercator(λ, φ) {
  ...
  (d3v3.geo.mercator = function() {
    return d3v3_geo_projection(d3v3_geo_mercator).scale(500);
  ...
  d3v3.geo.path = function() {
    ...
    path.centroid = function(object) {
      d3v3_geo_centroidDimension = d3v3_geo_centroidX = d3v3_geo_centroidY = d3v3_geo_centroidZ = 0;
      d3v3.geo.stream(object, projectStream(d3v3_geo_pathCentroid));
      return d3v3_geo_centroidZ ? [ d3v3_geo_centroidX / d3v3_geo_centroidZ, d3v3_geo_centroidY / d3v3_geo_centroidZ ] : undefined;
    };
    path.bounds = function(object) {
      return d3v3_geo_bounds(projectStream)(object);
    };
  ...
  function d3v3_geo_pathCircle(radius) {
    return "m0," + radius + "a" + radius + "," + radius + " 0 1,1 0," + -2 * radius + "a" + radius + "," + radius + " 0 1,1 0," + +2 * radius + "z";
  }
  function d3v3_geo_pathProjectStream(project) {
    var resample = d3v3_geo_resample(function(λ, φ) {
      return project([ λ * d3v3_degrees, φ * d3v3_degrees ]);
    });
    return function(stream) {
      stream = resample(stream);
      return {
        point: function(λ, φ) {
          stream.point(λ * d3v3_radians, φ * d3v3_radians);
        },
        sphere: function() {
          stream.sphere();
        },
  ...
  function d3v3_geo_pathBuffer() {
    var pointCircle = d3v3_geo_pathCircle(4.5), buffer = [];
    var stream = {
      point: point,
      lineStart: function() {
      ...
      pointRadius: function(_) {
        pointCircle = d3v3_geo_pathCircle(_);
        return stream;
      },
      result: function() {
        if (buffer.length) {
  ...
  var d3v3_geo_pathAreaSum, d3v3_geo_pathAreaPolygon, d3v3_geo_pathArea = {
    ...
    polygonEnd: function() {
      ...
      d3v3_geo_pathAreaSum += Math.abs(d3v3_geo_pathAreaPolygon / 2);
    }
  };
  ...
  function d3v3_geo_pathCentroidPoint(x, y) {
    if (d3v3_geo_centroidDimension) return;
    d3v3_geo_centroidX += x;
    d3v3_geo_centroidY += y;
    ++d3v3_geo_centroidZ;
  }
  function d3v3_geo_pathCentroidLineStart() {
    ...
    if (d3v3_geo_centroidDimension !== 1) {
      if (d3v3_geo_centroidDimension < 1) {
        d3v3_geo_centroidDimension = 1;
        d3v3_geo_centroidX = d3v3_geo_centroidY = d3v3_geo_centroidZ = 0;
      } else return;
    }
    ...
    function nextPoint(x, y) {
      ...
      d3v3_geo_centroidX += z * (x0 + x) / 2;
      d3v3_geo_centroidY += z * (y0 + y) / 2;
      d3v3_geo_centroidZ += z;
      x0 = x, y0 = y;
    }
  }
  ...
  function d3v3_geo_pathCentroidRingStart() {
    ...
    function nextPoint(x, y) {
      var z = y0 * x - x0 * y;
      d3v3_geo_centroidX += z * (x0 + x);
      d3v3_geo_centroidY += z * (y0 + y);
      d3v3_geo_centroidZ += z * 3;
      x0 = x, y0 = y;
    }
  ...
  var d3v3_geo_areaSum, d3v3_geo_areaRingU, d3v3_geo_areaRingV;
  var d3v3_geo_area = {
    sphere: function() {
      d3v3_geo_areaSum += 4 * π;
    },
    point: d3v3_noop,
    lineStart: d3v3_noop,
    lineEnd: d3v3_noop,
    polygonStart: function() {
      d3v3_geo_areaRingU = 1, d3v3_geo_areaRingV = 0;
      d3v3_geo_area.lineStart = d3v3_geo_areaRingStart;
    },
    polygonEnd: function() {
      var area = 2 * Math.atan2(d3v3_geo_areaRingV, d3v3_geo_areaRingU);
      d3v3_geo_areaSum += area < 0 ? 4 * π + area : area;
      d3v3_geo_area.lineStart = d3v3_geo_area.lineEnd = d3v3_geo_area.point = d3v3_noop;
    }
  };
  function d3v3_geo_areaRingStart() {
    var λ00, φ00, λ0, cosφ0, sinφ0;
    d3v3_geo_area.point = function(λ, φ) {
      d3v3_geo_area.point = nextPoint;
      λ0 = (λ00 = λ) * d3v3_radians, cosφ0 = Math.cos(φ = (φ00 = φ) * d3v3_radians / 2 + π / 4), 
      sinφ0 = Math.sin(φ);
    };
    function nextPoint(λ, φ) {
      λ *= d3v3_radians;
      φ = φ * d3v3_radians / 2 + π / 4;
      var dλ = λ - λ0, cosφ = Math.cos(φ), sinφ = Math.sin(φ), k = sinφ0 * sinφ, u0 = d3v3_geo_areaRingU, v0 = d3v3_geo_areaRingV, u = cosφ0 * cosφ + k * Math.cos(dλ), v = k * Math.sin(dλ);
      d3v3_geo_areaRingU = u0 * u - v0 * v;
      d3v3_geo_areaRingV = v0 * u + u0 * v;
  ...
  function d3v3_geo_projectionMutator(projectAt) {
    ...
    }), k = 150, x = 480, y = 250, λ = 0, φ = 0, δλ = 0, δφ = 0, δγ = 0, δx, δy, clip = d3v3_geo_clipAntimeridian, clipAngle = null;
    function projection(point) {
    ...
    projection.stream = function(stream) {
      return d3v3_geo_projectionRadiansRotate(rotate, clip(projectResample(stream)));
    };
    projection.clipAngle = function(_) {
      if (!arguments.length) return clipAngle;
      clip = _ == null ? (clipAngle = _, d3v3_geo_clipAntimeridian) : d3v3_geo_clipCircle(clipAngle = +_);
      return projection;
    };
    ...
    projection.center = function(_) {
      if (!arguments.length) return [ λ * d3v3_degrees, φ * d3v3_degrees ];
      λ = _[0] % 360 * d3v3_radians;
      φ = _[1] % 360 * d3v3_radians;
      return reset();
    };
    projection.rotate = function(_) {
      if (!arguments.length) return [ δλ * d3v3_degrees, δφ * d3v3_degrees, δγ * d3v3_degrees ];
      δλ = _[0] % 360 * d3v3_radians;
      δφ = _[1] % 360 * d3v3_radians;
      δγ = _.length > 2 ? _[2] % 360 * d3v3_radians : 0;
      return reset();
    };
    d3v3.rebind(projection, projectResample, "precision");
    function reset() {
      projectRotate = d3v3_geo_compose(rotate = d3v3_geo_rotation(δλ, δφ, δγ), project);
  ...
  function d3v3_geo_projectionRadiansRotate(rotate, stream) {
    return {
      point: function(x, y) {
        y = rotate(x * d3v3_radians, y * d3v3_radians), x = y[0];
  ...
  function d3v3_geo_rotation(δλ, δφ, δγ) {
    return δλ ? δφ || δγ ? d3v3_geo_compose(d3v3_geo_rotationλ(δλ), d3v3_geo_rotationφγ(δφ, δγ)) : d3v3_geo_rotationλ(δλ) : δφ || δγ ? d3v3_geo_rotationφγ(δφ, δγ) : d3v3_geo_equirectangular;
  }
  function d3v3_geo_forwardRotationλ(δλ) {
    return function(λ, φ) {
      return λ += δλ, [ λ > π ? λ - 2 * π : λ < -π ? λ + 2 * π : λ, φ ];
    };
  }
  function d3v3_geo_rotationλ(δλ) {
    var rotation = d3v3_geo_forwardRotationλ(δλ);
    rotation.invert = d3v3_geo_forwardRotationλ(-δλ);
    return rotation;
  }
  function d3v3_geo_rotationφγ(δφ, δγ) {
  ...
  var d3v3_geo_stereographic = d3v3_geo_azimuthal(function(cosλcosφ) {
    return 1 / (1 + cosλcosφ);
  }, function(ρ) {
  ...
  d3v3.geom.hull = function(vertices) {
    ...
    for (;j < plen; ++j) {
      ...
      while (!d3v3_geom_hullCCW(stack[sp - 2], stack[sp - 1], points[j].index, vertices)) {
        --sp;
      }
  ...
  function d3v3_geom_hullCCW(i1, i2, i3, v) {
    var t, a, b, c, d, e, f;
    t = v[i1];
    a = t[0];
    b = t[1];
  ...
  d3v3.geom.voronoi = function(vertices) {
    ...
    d3v3_voronoi_tessellate(vertices, function(e) {
  ...
  var d3v3_voronoi_opposite = {
    l: "r",
    r: "l"
  };
  function d3v3_voronoi_tessellate(vertices, callback) {
    var Sites = {
    ...
    var EdgeList = {
      ...
      rightRegion: function(he) {
        return he.edge == null ? Sites.bottomSite : he.edge.region[d3v3_voronoi_opposite[he.side]];
      }
    };
    var Geom = {
      ...
      endPoint: function(edge, side, site) {
        edge.ep[side] = site;
        if (!edge.ep[d3v3_voronoi_opposite[side]]) return;
        callback(edge);
      },
    ...
    while (true) {
      ...
      if (newSite && (EventQueue.empty() || newSite.y < newIntStar.y || newSite.y == newIntStar.y && newSite.x < newIntStar.x)) {
        ...
        Geom.endPoint(e, d3v3_voronoi_opposite[pm], v);
  ...
  d3v3.geom.quadtree = function(points, x1, y1, x2, y2) {
    ...
    root.visit = function(f) {
      d3v3_geom_quadtreeVisit(f, root, x1, y1, x2, y2);
    };
  ...
  d3v3.time = {};
  var d3v3_time = Date, d3v3_time_daySymbols = [ "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" ];
  function d3v3_time_utc() {
    this._ = new Date(arguments.length > 1 ? Date.UTC.apply(this, arguments) : arguments[0]);
  }
  d3v3_time_utc.prototype = {
    getDate: function() {
  ...
  var d3v3_time_formatDateTime = "%a %b %e %X %Y", d3v3_time_formatDate = "%m/%d/%Y", d3v3_time_formatTime = "%H:%M:%S";
  var d3v3_time_days = [ "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" ], d3v3_time_dayAbbreviations = [ "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" ], d3v3_time_months = [ "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" ], d3v3_time_monthAbbreviations = [ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" ];
  ...
  function d3v3_time_parse(date, template, string, j) {
    ...
    while (i < n) {
      ...
      if (c === 37) {
        p = d3v3_time_parsers[template.charAt(i++)];
  ...
  var d3v3_time_dayRe = d3v3_time_formatRe(d3v3_time_days), d3v3_time_dayAbbrevRe = d3v3_time_formatRe(d3v3_time_dayAbbreviations), d3v3_time_monthRe = d3v3_time_formatRe(d3v3_time_months), d3v3_time_monthLookup = d3v3_time_formatLookup(d3v3_time_months), d3v3_time_monthAbbrevRe = d3v3_time_formatRe(d3v3_time_monthAbbreviations), d3v3_time_monthAbbrevLookup = d3v3_time_formatLookup(d3v3_time_monthAbbreviations);
  ...
  var d3v3_time_formats = {
    a: function(d) {
      return d3v3_time_dayAbbreviations[d.getDay()];
    },
    A: function(d) {
      return d3v3_time_days[d.getDay()];
    },
    b: function(d) {
      return d3v3_time_monthAbbreviations[d.getMonth()];
    },
    B: function(d) {
      return d3v3_time_months[d.getMonth()];
    },
    c: d3v3.time.format(d3v3_time_formatDateTime),
    d: function(d, p) {
    ...
    j: function(d, p) {
      return d3v3_time_formatPad(1 + d3v3.time.dayOfYear(d), p, 3);
    },
    L: function(d, p) {
    ...
    U: function(d, p) {
      return d3v3_time_formatPad(d3v3.time.sundayOfYear(d), p, 2);
    },
    w: function(d) {
      return d.getDay();
    },
    W: function(d, p) {
      return d3v3_time_formatPad(d3v3.time.mondayOfYear(d), p, 2);
    },
    x: d3v3.time.format(d3v3_time_formatDate),
    X: d3v3.time.format(d3v3_time_formatTime),
    y: function(d, p) {
  ...
  function d3v3_time_parseMonth(date, string, i) {
    ...
    var n = d3v3_time_monthRe.exec(string.substring(i));
  ...
  function d3v3_time_parseFullYear(date, string, i) {
    ...
    var n = d3v3_time_numberRe.exec(string.substring(i, i + 4));
    return n ? (date.y = +n[0], i += n[0].length) : -1;
  }
  function d3v3_time_parseYear(date, string, i) {
    d3v3_time_numberRe.lastIndex = 0;
    var n = d3v3_time_numberRe.exec(string.substring(i, i + 2));
  ...
  function d3v3_time_parseMilliseconds(date, string, i) {
    d3v3_time_numberRe.lastIndex = 0;
    var n = d3v3_time_numberRe.exec(string.substring(i, i + 3));
    return n ? (date.L = +n[0], i += n[0].length) : -1;
  }
  var d3v3_time_numberRe = /^\s*\d+/;
  function d3v3_time_parseAmPm(date, string, i) {
    var n = d3v3_time_amPmLookup.get(string.substring(i, i += 2).toLowerCase());
    return n == null ? -1 : (date.p = n, i);
  }
  var d3v3_time_amPmLookup = d3v3.map({
    am: 0,
    pm: 1
  });
  ...
  d3v3.time.format.utc = function(template) {
    ...
    function format(date) {
      try {
        d3v3_time = d3v3_time_utc;
        var utc = new d3v3_time();
        utc._ = date;
        return local(utc);
      } finally {
        d3v3_time = Date;
  ...
  var d3v3_time_formatIso = d3v3.time.format.utc("%Y-%m-%dT%H:%M:%S.%LZ");
  d3v3.time.format.iso = Date.prototype.toISOString ? d3v3_time_formatIsoNative : d3v3_time_formatIso;
  ...
  function d3v3_time_interval(local, step, number) {
    ...
    function ceil(date) {
      step(date = local(new d3v3_time(date - 1)), 1);
      return date;
    }
    function offset(date, k) {
      step(date = new d3v3_time(+date), k);
      return date;
    }
  ...
  d3v3.time.second = d3v3_time_interval(function(date) {
    return new d3v3_time(Math.floor(date / 1e3) * 1e3);
  ...
  d3v3.time.minute = d3v3_time_interval(function(date) {
    return new d3v3_time(Math.floor(date / 6e4) * 6e4);
  }, function(date, offset) {
  ...
  d3v3.time.hour = d3v3_time_interval(function(date) {
    var timezone = date.getTimezoneOffset() / 60;
    return new d3v3_time((Math.floor(date / 36e5 - timezone) + timezone) * 36e5);
  }, function(date, offset) {
  ...
  d3v3.time.day = d3v3_time_interval(function(date) {
    var day = new d3v3_time(1970, 0);
  ...
  d3v3_time_daySymbols.forEach(function(day, i) {
    day = day.toLowerCase();
    i = 7 - i;
    var interval = d3v3.time[day] = d3v3_time_interval(function(date) {
  ...
  d3v3.time.month = d3v3_time_interval(function(date) {
  ...
  d3v3.time.year = d3v3_time_interval(function(date) {
    date = d3v3.time.day(date);
    date.setMonth(0, 1);
    return date;
  }, function(date, offset) {
  ...
  function d3v3_time_scale(linear, methods, format) {
    ...
    scale.nice = function(m) {
      return scale.domain(d3v3_scale_nice(scale.domain(), function() {
        return m;
      }));
    };
    scale.ticks = function(m, k) {
      var extent = d3v3_time_scaleExtent(scale.domain());
      if (typeof m !== "function") {
        var span = extent[1] - extent[0], target = span / m, i = d3v3.bisect(d3v3_time_scaleSteps, target);
        if (i == d3v3_time_scaleSteps.length) return methods.year(extent, m);
        if (!i) return linear.ticks(m).map(d3v3_time_scaleDate);
        if (Math.log(target / d3v3_time_scaleSteps[i - 1]) < Math.log(d3v3_time_scaleSteps[i] / target)) --i;
        m = methods[i];
        k = m[1];
        m = m[0].range;
      }
  ...
  function d3v3_time_scaleExtent(domain) {
    var start = domain[0], stop = domain[domain.length - 1];
    return start < stop ? [ start, stop ] : [ stop, start ];
  }
  ...
  function d3v3_time_scaleFormat(formats) {
  ...
  function d3v3_time_scaleSetYear(y) {
    var d = new Date(y, 0, 1);
    d.setFullYear(y);
    return d;
  }
  function d3v3_time_scaleGetYear(d) {
    var y = d.getFullYear(), d0 = d3v3_time_scaleSetYear(y), d1 = d3v3_time_scaleSetYear(y + 1);
    return y + (d - d0) / (d1 - d0);
  }
  ...
  var d3v3_time_scaleLocalMethods = [ [ d3v3.time.second, 1 ], [ d3v3.time.second, 5 ], [ d3v3.time.second, 15 ], [ d3v3.time.second, 30 ], [ d3v3.time.minute, 1 ], [ d3v3.time.minute, 5 ], [ d3v3.time.minute, 15 ], [ d3v3.time.minute, 30 ], [ d3v3.time.hour, 1 ], [ d3v3.time.hour, 3 ], [ d3v3.time.hour, 6 ], [ d3v3.time.hour, 12 ], [ d3v3.time.day, 1 ], [ d3v3.time.day, 2 ], [ d3v3.time.week, 1 ], [ d3v3.time.month, 1 ], [ d3v3.time.month, 3 ], [ d3v3.time.year, 1 ] ];
  var d3v3_time_scaleLocalFormats = [ [ d3v3.time.format("%Y"), d3v3_true ], [ d3v3.time.format("%B"), function(d) {
    return d.getMonth();
  ...
  var d3v3_time_scaleLinear = d3v3.scale.linear(), d3v3_time_scaleLocalFormat = d3v3_time_scaleFormat(d3v3_time_scaleLocalFormats);
  d3v3_time_scaleLocalMethods.year = function(extent, m) {
    return d3v3_time_scaleLinear.domain(extent.map(d3v3_time_scaleGetYear)).ticks(m).map(d3v3_time_scaleSetYear);
  };
  ...
  var d3v3_time_scaleUTCFormats = [ [ d3v3.time.format.utc("%Y"), d3v3_true ], [ d3v3.time.format.utc("%B"), function(d) {
  ...
  var d3v3_time_scaleUTCFormat = d3v3_time_scaleFormat(d3v3_time_scaleUTCFormats);
  function d3v3_time_scaleUTCSetYear(y) {
    var d = new Date(Date.UTC(y, 0, 1));
    d.setUTCFullYear(y);
    return d;
  }
  function d3v3_time_scaleUTCGetYear(d) {
    var y = d.getUTCFullYear(), d0 = d3v3_time_scaleUTCSetYear(y), d1 = d3v3_time_scaleUTCSetYear(y + 1);
    return y + (d - d0) / (d1 - d0);
  }
  d3v3_time_scaleUTCMethods.year = function(extent, m) {
    return d3v3_time_scaleLinear.domain(extent.map(d3v3_time_scaleUTCGetYear)).ticks(m).map(d3v3_time_scaleUTCSetYear);
  };
